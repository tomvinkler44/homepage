      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h2 className="text-4xl font-bold mb-4">
            Our Members Get <span className="text-orange-500">Real Results</span> (Even in This Job Market)
          </h2>
          <p className="text-xl text-gray-600">
            While other job seekers are submitting hundreds of applications with no response, 
            our members are getting interviews and offers
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-4 gap-8">
          {stats.map((stat, index) => {
            ...
            return (
              ...
              >
                <div className={`w-16 h-16 ${colorClasses[stat.color]} rounded-full flex items-center justify-center mx-auto mb-4`}>
                  <Icon className="w-8 h-8" />
                </div>
                <div className={`text-5xl font-bold mb-2 ${`text-${stat.color}-600`}`}>{stat.number}</div>
                <div className="text-gray-600">{stat.description}</div>
                <div className="mt-4 w-full bg-gray-200 h-2 rounded-full overflow-hidden">
                  <motion.div 
                    className={`h-full ${`bg-${stat.color}-500`}`}
                    initial={{ width: "0%" }}
                    whileInView={{ width: stat.number.replace(/[^0-9]/g, '') + "%" }}
